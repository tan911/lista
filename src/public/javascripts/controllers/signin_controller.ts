...
export default class SigninController extends Controller<HTMLFormElement> {
    ...
    async submit(event: Event) {
        ...
        const dataSanitization = LoginSchema.safeParse({
        ...
        try {
            if (dataSanitization.success) {
                ...
                const response = await user.sync('login', data)
